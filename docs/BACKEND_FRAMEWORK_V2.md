...
graph TB
    subgraph Client["客户端层 (Client Layer)"]
        ...
        Web["Web 管理台<br/>Vue3/React"]
    end

    subgraph Access["接入层 (Access Layer)"]
        Nginx["Nginx 反向代理<br/>SSL 终止 / 负载均衡 / 缓存"]
    end

    subgraph Gateway["API 网关层 (API Gateway)"]
    ...
    subgraph App["应用服务层 (Application Layer)"]
        AuthService["用户服务<br/>AuthService"]
        RecordService["档案服务<br/>RecordService"]
        IndicatorService["指标服务<br/>IndicatorSvc"]
        MedService["用药服务<br/>MedService"]
        OCRService["OCR 服务<br/>OCRService"]
        AIService["AI 分析服务<br/>AIService"]
        NotifyService["通知服务<br/>NotifyService"]
        FileService["文件服务<br/>FileService"]
    end

    subgraph Data["数据持久层 (Data Persistence)"]
        MySQL["MySQL 8.0<br/>关系型数据"]
        MongoDB["MongoDB 6.0<br/>文档型数据"]
        Redis["Redis 7.0<br/>缓存/队列"]
        OSS["对象存储<br/>文件存储"]
    end

    iOS -->|HTTPS TLS 1.3| Nginx
    Android -->|HTTPS TLS 1.3| Nginx
    Web -->|HTTPS TLS 1.3| Nginx
    ...
    FastAPI --> IndicatorService
    FastAPI --> MedService
    FastAPI --> OCRService
    FastAPI --> AIService
    FastAPI --> NotifyService
    FastAPI --> FileService
    
    AuthService --> MySQL
    RecordService --> MySQL
    IndicatorService --> MySQL
    MedService --> MySQL
    AIService --> MongoDB
    NotifyService --> Redis
    FileService --> OSS
...
sequenceDiagram
    ...
    Client->>API: POST /auth/register<br/>(phone, password, verify_code)
    API->>Redis: 验证短信验证码
    Redis-->>API: 验证码有效
    API->>Auth: 检查手机号是否已注册
    Auth->>MySQL: SELECT * FROM users WHERE phone=?
    MySQL-->>Auth: 返回查询结果
    Auth-->>API: 手机号未注册
    API->>Auth: 创建新用户
    Auth->>MySQL: INSERT INTO users (...)
    MySQL-->>Auth: 返回用户 ID
    Auth->>Auth: 密码 bcrypt 加密
    Auth-->>API: 返回用户 UUID
    API-->>Client: 注册成功 {uuid}
    ...
    API->>Redis: 存储 Session (TTL 7 天)
    API-->>Client: 登录成功 {token, user}
```
...
```mermaid
flowchart TD
    Start([开始]) --> Upload[客户端上传文件]
    Upload --> Validate{文件验证}
    Validate -->|格式错误 | Reject[拒绝上传]
    Validate -->|验证通过 | CreateRecord[创建档案记录]
    CreateRecord --> UploadOSS[上传到 OSS]
    UploadOSS --> GetURL[获取文件 URL]
    GetURL --> UpdateDB[更新档案 file_urls]
    UpdateDB --> TriggerOCR[触发 OCR 异步任务]
    TriggerOCR --> Queue[Celery 任务队列]
    
    Queue --> OCRWorker[OCR Worker 取任务]
    OCRWorker --> CallOCR[调用百度/腾讯 OCR API]
    CallOCR --> ParseOCR{解析结果}
    ParseOCR -->|失败 | Retry{重试?}
    Retry -->|是 | CallOCR
    Retry -->|否 | MarkFailed[标记 OCR 失败]
    ParseOCR -->|成功 | ExtractData[提取结构化数据]
    ExtractData --> UpdateStatus[更新档案 OCR 状态]
    UpdateStatus --> SaveResult[保存 OCR 结果]
    SaveResult --> End([结束])
    
    Reject --> End
    MarkFailed --> End
    
    style Start fill:#52c41a
    style End fill:#52c41a
    style Reject fill:#ff4d4f
    style MarkFailed fill:#ff9c6e
```
...
sequenceDiagram
    ...
    Client->>API: GET /indicators/trend?<br/>indicator_code=BP_HIGH&<br/>member_id=xxx&<br/>start_date=2025-01-01&<br/>end_date=2026-03-08
    API->>Redis: 查询缓存
    Redis-->>API: 缓存未命中
    API->>Indicator: 获取指标趋势
    Indicator->>MySQL: SELECT * FROM health_indicators<br/>WHERE user_id=? AND<br/>indicator_code=? AND<br/>measure_date BETWEEN ? AND ?
    MySQL-->>Indicator: 返回指标数据
    Indicator->>Indicator: 计算统计值<br/>(avg, min, max)
    Indicator->>Redis: 缓存结果 (TTL 1 小时)
    Indicator-->>API: 返回趋势数据
